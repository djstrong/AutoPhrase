...
TOKENIZER="-cp .:tools/tokenizer/lib/*:tools/tokenizer/resources/:tools/tokenizer/build/ Tokenizer -l PL"
...
# Tworzone są 4 pliki w katalogu tmp:
...
# tokenized_train.txt:
# 2 357 134 414 369 794 266 1461 1653 1315 369 1653 . 199 1029 ...

# case_tokenized_train.txt:
# 0404000001000300311030311031130330110000404000000333300000000404003333300000003000404000000303331434000...

# przy PL brakuje raw_tokenized_train.txt? nie

# pos_tags_tokenized_train.txt:
# subst adj subst adj subst adj subst adj prep ...
...
if [ $FIRST_RUN -eq 1 ]; then
    ...
    # raw_tokenized_all.txt
    # uniwersytet ekonomiczny w Kraków
    # plac łukiski
    # kategoria podgatunek muzyka house
    # gaić
    
    
    java $TOKENIZER -m test -i $QUALITY_WIKI_ENTITIES -o $TOKENIZED_QUALITY -t $TOKEN_MAPPING -c N -thread $THREAD
    # tokenized_quality.txt
    # raw_tokenized_quality.txt
...
if [ ! $LANGUAGE == "JA" ] && [ ! $LANGUAGE == "CN" ]  && [ ! $LANGUAGE == "PL" ] && [ ! $LANGUAGE == "OTHER" ]  && [ $ENABLE_POS_TAGGING -eq 1 ] && [ $FIRST_RUN -eq 1 ]; then
    ...
    # tmp/pos_tags_tokenized_train.txt
    # NN
    # SYM
    # NP
    # SYM
